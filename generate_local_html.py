...
function handleFolderSelect(files) {
    ...
    let processed = 0, totalInput = 0, totalOutput = 0, totalCached = 0;
    const weekData = {};

    for (const file of files) {
        if (!file.name.endsWith('.json')) { processed++; continue; }
...
function finishParsing() {
    ...
    currentData = {
        period: start+' ~ '+end, weeks, summary:{
            total, totalInput, totalOutput, totalCached,
            cacheRate: Math.round(totalCached/max(total,1)*100*10)/10,
            activeWeeks: weeks.length,
            dailyAvg: Math.round(total/max(days,1)),
            days
        }
    };
...
function generateAnalysis(d) {
    ...
    document.getElementById('analysisContent').innerHTML = '<div class=tips><h4>📊 使用分析</h4><ul><li><strong>使用频率：</strong>在 '+(summary.days||0)+' 天内共有 '+weeks.length+' 周使用记录</li><li><strong>周均使用：</strong>'+(avg/1000000).toFixed(1)+'M tokens</li><li><strong>缓存效率：</strong>'+(summary.cacheRate||0)+'%</li><li><strong>趋势：</strong>'+(trend||'数据不足')+'</li></ul><h4 style=margin-top:16px>🎯 优化建议</h4><ul><li>保持同一主题对话连贯性，提高缓存复用率</li><li>将相关任务放在一起处理</li><li>完成的主题及时开启新会话</li></ul></div>';
}
...
function resetPage() {
    ...
    document.getElementById('changeFolder').classList.add('hidden');
...
output_path = Path.home() / 'Downloads' / 'workbuddy-token-stats-local.html'
output_path.write_text(html)
print(f'✅ 本地版已生成: {output_path}')
